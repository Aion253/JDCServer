...
public class ContextHandler implements HttpHandler {
	...
	public void handle(HttpExchange he) {
		...
		String hostName = he.getRequestHeaders().getFirst("Host");
		Website wb = WebsiteManager.getWebsiteByAddress(hostName);
		if(requestSplit[0].endsWith(".jdc")) {
			ResponseUtils.generateHTTPResponse(he, 200, PageParser.parseGeneratePage(wb, he, postQuery, getQuery, wb.getContentFile(requestSplit[0])));
			return;
		} else {
			ResponseUtils.fileHTTPResponse(he, wb.getContentFile(requestSplit[0]));
			return;
		}
	}
